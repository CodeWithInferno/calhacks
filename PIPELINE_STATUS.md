...
| 3. Training pipeline | Train supervised GRU/MLP world model on real physics data | Trained on VM with CUDA; episode-split split avoids leakage. `models/mujoco_large_split`: **val AUC 0.9925 / test AUC 0.9950 / test F1 0.9728** | ✅ Trained |
...
- The trained world model is in `models/mujoco_large_split/` on the VM and has been copied locally. It was retrained with an episode-wise train/val/test split so that windows from the same episode do not leak across splits.
