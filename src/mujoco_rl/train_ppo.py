...
def make_env(slope_deg: float, friction: float, cmd_vel: tuple[float, float], seed: int = 0):
    def _init():
        env = G1WalkEnv(
            slope_deg=slope_deg,
            friction=friction,
...
def main():
    ...
    parser.add_argument("--output", default="models/g1_ppo_walk")
    ...
    parser.add_argument("--cmd-vel", nargs=2, type=float, default=[0.5, 0.0])
    ...
    env_fns = [
        make_env(args.slope_deg, args.friction, args.cmd_vel, seed=i)
